...
#### P8.A.1 Plugin package skeleton -- DONE (commit 91bcb4c)
...
#### P8.A.2 SCL parser -- Edition 2.1 schema-validating loader -- DONE (commit 92c88df, 46 tests)
...
#### P8.A.3 SCL diff engine -- DONE (commit 1d4b636, 25 tests)
...
#### P8.B.1 Choose the MMS library -- DONE (pyiec61850 / libiec61850 SWIG; ADR in docs/internal/IEC61850_MMS_LIBRARY.md)
...
#### P8.B.2 MMS connect / abort / status -- DONE (commit dd6c27f)

* Adapter: `_mms_client.py` -- `MmsClient`, `MmsConnectError`.
* `connect()` in `driver.py` uses `MmsClient`; session exposes
  `negotiated_pdu_size` and `peer_implementation`.
* 28 unit tests in `test_iec61850_plugin.py`.
...
#### P8.B.3 Data model browse -- DONE (commit adf3166)

* `MmsClient.get_server_directory()`, `get_logical_device_directory()`,
  `get_logical_node_directory()` added to `_mms_client.py`.
* `enumerate_objects()` walks server -> LD -> LN -> DO; yields
  `ObjectRef(data_type="do")` per DO; skips on `MmsDirectoryError`.
* 7 new unit tests (`TestEnumerateObjects`).
...
#### P8.B.4 Read with quality and timestamp -- DONE (this session)

* FC constants (`FC_ST`, `FC_MX`, `FC_SP`, ...) and `MmsDecodedValue`
  dataclass added to `_mms_client.py`.
* `MmsClient.read_object()`, `read_do_with_meta()` added; quality bit
  decoding via `MmsValue_getBitStringBit` -> `(validity, substituted, test)`.
* `read()` in `driver.py` tries FC_MX -> FC_ST -> FC_SP for DO-level
  refs; uses explicit FC for ``[FC]``-suffixed DA refs; never raises
  (returns BAD `ReadResult` on failure).
* 10 new unit tests (`TestRead`).

* **Goal:** `read(path)` returns `ReadResult(value, quality,
  time_quality, ts_ns, source)` per IEC 61850-7-2 §6.

#### P8.B.5 Write — direct / SBO, normal / enhanced security -- DONE (commit 0122576)

* All four control models implemented via `ControlObjectClient_*` libiec61850 API.
* `MmsClient.write_control(do_ref, value, oper_time_ms)` → `ControlResult`
  handles model detection, SBO select/selectWithValue, operate, and
  `getLastApplError` (AddCause extraction).
* `prepare_write` validates ctlVal type (bool/int/float), returns `WriteIntent`
  with no I/O.
* `commit_write` calls `safety.require_write_authorization`, delegates to
  `write_control`, populates `WriteResult.metadata` with
  `control_model`/`add_cause`/`add_cause_name`, and calls
  `safety.record_write_outcome` for every attempted transmission.
* 14 new unit tests (`TestPrepareWrite` + `TestCommitWrite`).
...
#### P8.B.6 Reporting (BRCB/URCB) -- DONE (commit 21f866b, 75 tests)

* `MmsClient.get_rcb_values()` reads RCB attributes → `RcbValues`.
* `MmsClient.enable_report()` installs `IedConnection_installReportHandler`
  callback, sets `RptEna=True` via `IedConnection_setRCBValues`.
* `MmsClient.disable_report()` sets `RptEna=False`, clears handler ref (GC-safe).
* `Iec61850MmsSession.subscribe_report()` / `unsubscribe_report()` delegate
  to `MmsClient`; raise `MmsDirectoryError` when no active client.

#### P8.B.7 Logging (read log buffer)

**DONE** — commit `74744f4`, 85 tests.

* `LogEntry` dataclass: `log_ref`, `entry_id` (bytes), `occurrence_time_ms`, `variables`.
* `MmsClient._decode_journal_entries()` traverses `LinkedList<MmsJournalEntry>`,
  decodes occurrence time (`MmsValue_getBinaryTimeAsUtcMs`), entry ID (octet string),
  and journal variables; frees list in `finally` via `LinkedList_destroyDeep`.
* `MmsClient.query_log_by_time(log_ref, start_ms, end_ms)` → `(list[LogEntry], bool)`
  wraps `IedConnection_queryLogByTime`.
* `MmsClient.query_log_after(log_ref, entry_id, timestamp_ms)` → `(list[LogEntry], bool)`
  wraps `IedConnection_queryLogAfter`; builds `MmsValue` for entry ID, frees in `finally`.
* `Iec61850MmsSession.query_log_by_time()` / `query_log_after()` delegate to client;
  raise `MmsDirectoryError` when no active client.
